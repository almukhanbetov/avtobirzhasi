...
func main() {
	...
	carsHandler := handlers.NewCarsHandler(listingRepo, locationRepo)
	locationsHandler := handlers.NewLocationsHandler(locationRepo)
	sellersHandler := handlers.NewSellersHandler(userRepo, listingRepo)
	listingsHandler := handlers.NewListingsHandler(listingRepo, locationRepo)
	requestsHandler := handlers.NewRequestsHandler(buyerRequestRepo, locationRepo)
	...
	adminListingsHandler := handlers.NewAdminListingsHandler(listingRepo, userRepo, locationRepo)
	...
	handlers.RegisterLocationsRoutes(api, locationsHandler)
